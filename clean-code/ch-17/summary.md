...
### G20: 이름과 기능이 일치하는 함수
* 이름만으로 기능이 분명하게 유추되지 않는 함수가 있다면, 더 좋은 이름으로 바꾸거나 더 좋은 이름을 붙이기 쉽도록 기능을 간소화해야 한다.

### G21: 알고리즘을 이해하라
* 자신이 알고리즘을 정확하게 이해하고, 올바르다는 사실을 알아야 한다.
* 테스트 케이스를 모두 통과한다고 해서 함수를 온전히 완성한 것이 아니다.

### G22: 논리적 의존성은 물리적으로 드러내라
* 한 모듈이 다른 모듈에 논리적으로 의존한다면 이를 물리적으로, 즉 명시적으로 표현해야 한다.

### G23: If/Else, Switch/Case문보다 다형성을 사용하라
* Switch문이 최선인 경우도 있지만, switch문을 선택하기 전에 언제나 다형성을 우선으로 고려해야 한다.

### G24: 표준 표기법을 따르라
* 팀은 업계 표준에 기반한 구현 표준을 따라야 한다.
* 표준을 설명하는 문서는 코드 자체로 충분해야 하며 별도 문서를 만들 필요는 없어야 한다.

### G25: 매직 숫자는 명명된 상수로 표기하라
* 의미가 불분명한 토큰(숫자, 문자열 무엇이든)은 명명된 상수로 정의해야 한다.
...
### G27: 관례보다 구조를 사용하라
* 설계 결정을 강제할 때는 규칙보다 관례를 사용해야 한다.
* 명명 관례보다는 구조(클래스) 자체로 강제하면 더 좋다.

### G28: 조건을 캡슐화하라
* bool 논리는 조건의 의도를 분명히 밝히는 함수로 표현해야 한다.

### G29: 부정 조건은 피하라
* 긍정 조건이 부정 조건보다 이해하기 쉽기 때문에 부정 조건 대신 긍정 조건으로 표현하는 것이 좋다.

### G30: 함수는 한 가지만 해야 한다
* 한 가지만 수행하는 좀 더 작은 함수 여럿으로 나누어 함수를 구현해야 한다.

### G31: 숨겨진 시간적인 결합
* 함수 인수를 적절히 배치해 함수가 호출되는 순서를 명백히 드러내야 한다.
* 순서를 바꿔 호출할 수 없도로 명시적으로 복잡성을 표현해야 한다.

### G32: 일관성을 유지하라
* 코드 구조를 잡을 때는 이유를 명백하게 생각하고 이를 코드에 표현하여 일관성을 유지해야 한다.
* 일관성이 있는 시스템은 남들도 일관성을 따르기 때문에 일관성이 보존되기 쉽다.

### G33: 경계 조건을 캡슐화하라
* 경계 조건은 한 곳에서 별도로 처리해야 한다.

### G34: 함수는 추상화 수준을 한 단계만 내려야 한다.
* 함수 내모든 문장은 추상호 수준이 동일해야 한다.
* 추상화 수준 분리는 리팩터링을 수행하는 가장 중요한 이유 중 하나이면서, 제대로 하기에 가장 어려운 작업 중 하나이기도 하다.

### G35: 설정 정보는 최상위 단계에 둬라
* 설정 관련 상수는 최상위 단계에 둬야 찾기도 쉽고 변경하기도 쉽다.
* 저차원 함수에 상수값을 정의하면 안 된다.
